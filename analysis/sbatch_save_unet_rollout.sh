#!/bin/bash
# Save one PDEBench 2D CFD U-Net validation rollout under Slurm.
#
# Usage:
#   sbatch analysis/sbatch_save_unet_rollout.sh <REGIME_FILENAME> <ARTIFACT_ROOT>
...
python eval_unet_2dcfd_rollout.py \
  --filename "$REGIME" \
  --data_path "$DATA_DIR" \
  --out_dir "$OUT_DIR" \
